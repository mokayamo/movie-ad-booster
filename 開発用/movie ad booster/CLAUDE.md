...
This project requires a `GEMINI_API_KEY` environment variable set in `.env.local` for the Gemini API integration (configured in vite.config.ts).

## Architecture Overview

This is a React 19 + TypeScript + Vite single-page application for a Japanese video advertising optimization service ("動画広告ブースター").

### Project Structure
- `App.tsx` - Main application component that renders all sections sequentially
- `components/` - Self-contained React components for different page sections:
  - `Hero.tsx` - Landing hero section with main CTA
  - `ProblemSolution.tsx` - Problem/solution presentation
  - `ServiceDetails.tsx` - Service feature details
...
- `constants.tsx` - Shared SVG icons and UI constants
- `types.ts` - TypeScript type definitions
- `metadata.json` - Project metadata

### Key Patterns
- **Component Architecture**: Each section is a self-contained functional component imported into App.tsx
- **Styling**: Uses Tailwind CSS classes with custom design system colors (`#0D0D2B` dark theme, `#00E6A7`/`#00B4D8` gradient accents)
- **CTA Integration**: All CTAs link to `mailto:soudan@movie-ad-booster.example.com` through the CtaButton component
...
- **Environment Variables**: Gemini API key exposed as `process.env.GEMINI_API_KEY` and `process.env.API_KEY`

### Component Props Pattern
Components use TypeScript interfaces for props with optional className and specific prop types (e.g., `FaqItemType` interface in types.ts).
